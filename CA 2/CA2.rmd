...
sim_ar <- function(param){
  epsilon <- rnorm(n=1100, mean=0, sd=0.1)
  x <- epsilon[1:2]
  
  for(i in 3:length(epsilon)){
    x[i] <- t(param) %*% x[c(i-1, i-2)] + epsilon[i]
  }
  x <- x[-c(1:100)]
  return(ts(x))
}

sim_ma <- function(param){
  epsilon <- rnorm(n=1100, mean=0, sd=0.1)
  x <- epsilon[1:2]
  
  for(i in 3:length(epsilon)){
    x[i] <- t(param) %*% epsilon[c(i-1, i-2)] + epsilon[i]
  }
  x <- x[-c(1:100)]
  return(ts(x))
}
...
s <- c()
for(phi in param_list){
  sim <- replicate(3, sim_ar(phi))
  s <- cbind(s, sim)
}
for(theta in param_list){
  sim <- replicate(3, sim_ma(theta))
  s <- cbind(s, sim)
}
names <- paste0(rep(c("AR","MA"), each = 2), 1:2)
colnames(s) <- paste(rep(names, each = 3), rep(1:3, 4), sep = ".")
```
All models except the AR(2) model with parameters (0.8,0.2) define stationary stochastic processes. In case of the AR(2) model with parameters (0.8,0.2), the side condition of AR model parameters, namely φ1+φ2<1, is violated, resulting non-stationary time series.  
...
s %>% as.data.frame() %>%
      reshape(direction = "long", varying = list(1:3,4:6,7:9,10:12), v.names = names, timevar = "rep", idvar = "time") %>% 
      reshape(direction = "long", varying = 2:5, v.names = "sim", timevar = "setup", times = names, idvar = c("rep", "time")) %>%
      ggplot(aes(x = time, y = sim)) + geom_line() + facet_grid(setup ~ rep, scales = "free_y")
```
...
for(i in 1:ncol(s)){
  acf(s[,i], main=colnames(s)[i])
  pacf(s[,i], main=colnames(s)[i])
}
```
...
The KPSS test underlines that samples from the ARIMA process with the second parameter combination violate the stationary assumption.
...
arima_models <- apply(s, 2, auto.arima)
gt_param_list <- rep(rep(param_list, each = 3), 2)

for(i in 1:length(arima_models)){
  pred_param <- arima_models[[i]]$coef
  gt_param <- gt_param_list[[i]]
  if(i <= 6){
    names(gt_param) <- paste0("ar", 1:2)
  }
  else{
    names(gt_param) <- paste0("ma", 1:2)
  }
  
  res <- bind_rows(pred_param, gt_param) %>% 
    mutate(type = c("prediction", "gt")) %>%
    replace(is.na(.), 0)
  print(res)
}
```

In general, the estimated parameters of stationary AR(2) samples are very close to the true parameters. For MA(2) samples, restoring ground truth values (and hyperparameters) is less accurate. Estimation of additional AR or MA terms (in particular for MA(2) samples) might be due to local optima in the discrete optimization procedure.
...
Let φ1=0.8,φ2=0.2∈R be arbitrary coefficients of AR(2). The model is given as
xt=φ1xt−1+φ2xt−2+εt.

Applying a difference operator D leads to
D(xt)=xt−xt−1=φ1xt−1+φ2xt−2−xt−1+εt=(φ1−1)xt−1+φ2xt−2+εt=φ1~xt−1+φ2xt−2+εt

Thus, the differenced time series is an AR(2) model with parameters φ~1=φ1−1=−0.2 and φ2~=φ2=0.2, which fulfills the stated side conditions:

|φ~2|=0.2<1
(φ~1+φ~2)=(−0.2+0.2)=0<1
(φ~2−φ1~)=(0.2+0.2)=0.4<1
